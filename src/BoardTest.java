...
public class BoardTest {
	...
	public void testIsValidMove() {
		...
		for (int i = 0; i < board.boardPieces.length; i++) {
			for (int j = 0; j < board.boardPieces[i].length; j++) {
				if ((i == 2 && j == 4) || (i == 4 && j == 2) 
						|| (i == 5 && j == 3) 
						|| (i == 3 && j == 5)) {
	...
	/**
	 * Make the current player move and check that appropriate 
	 * cells has been flipped.
	 */
	@Test
	...
	public void testGameOver() {
		...
		for (int i = 0; i < board.boardPieces.length; i++) {
			for (int j = 0; j < board.boardPieces[i].length; j++) {
				board.boardPieces[i][j] = random.nextInt(2) + 1;
			}
		}
	...
	/**
	 * Starting a game by default would lead by default to have both 
	 * players to have a score of 2. As the games go on, the black
	 * score is updated.
	 */
	@Test
	...
	public void testCanMove() {
		...
		/* Switch the above scenario to make black 
	...
	/**
	 * This is a test case to test for winner between
	 *  white and black pieces.
	 */
	@Test
	public void testGetWinner() {
		//TestBoardPiecesSize should have arguments so
		//the method can be able to enter in other loops
		Board board = new Board();
		/*This is the only scenario that It can return, 
		 *suggestion to add 2 parameters in getWinner()
		 *getWinner(integer plyrOnePts, integer plyrTwoPts) then
		 *It can have 3 results 1, 2 and -1
		 */
